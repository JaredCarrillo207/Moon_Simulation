...
public class MoveGameObjectLoop : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        transform.localPosition = Vector3.Lerp(positions[currentPositionIndex], positions[(currentPositionIndex + 1) % positions.Count], fractionOfJourney);
        transform.localRotation = Quaternion.Lerp(rotations[currentRotationIndex], rotations[(currentRotationIndex + 1) % rotations.Count], fractionOfJourney);

        if (fractionOfJourney >= 1.0f)
        {
            currentPositionIndex = (currentPositionIndex + 1) % positions.Count;
            currentRotationIndex = (currentRotationIndex + 1) % rotations.Count;
            startTime = Time.time;
            journeyLength = Vector3.Distance(positions[currentPositionIndex], positions[(currentPositionIndex + 1) % positions.Count]);
        }

        if (!isLooping && currentPositionIndex == 0 && currentRotationIndex == 0 && fractionOfJourney >= 1.0f)
        {
            isLooping = true;
            Debug.Log("Looping CSV data...");
            ResetTimestamps();
        }
    }
